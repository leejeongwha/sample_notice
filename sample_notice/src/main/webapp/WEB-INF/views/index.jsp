...
3. 기능 요구사항
	...
	B. 공지사항 상세에서 저장버튼 클릭시에 세션 참조하여 로그인 되어있지 않을 경우 로그인 페이지로 리다이렉트
	C. 세션에 어드민ID가 존재할 경우(로그인되어있을 경우) 공지사항 저장 및 수정이 가능함
...
6. 추가 요구사항(선택사항)
	...
	C. 현재 정해진 admin만 글쓰기가 가능한 상태이나 회원가입 받아서 내가 쓴글에 대해서 삽입, 수정, 삭제 가능하도록 수정(자유게시판처럼)
	D. 오류 발생 시 현재는 에러코드가 그대로 화면에 노출되나 미리 설정된 ERROR 관련 페이지가 보여지도록 수정
</pre>
</body>
</html>